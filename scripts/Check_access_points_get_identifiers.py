...
""" Opens csv of access points and looks up access point in id.loc.gov, if it finds an authority record, it then checks for identifiers in VIAF and outputs 
text files (up to 50 lines long) that can be used in automated process in Connexion, and also outputs a summary spreadsheet, and a text file with any identifiers that don't match between LCNAF and VIAF"""

#looks up access point  
def Check_AP_LOC(lcnafAP):
    BASE_PR_URL = 'http://id.loc.gov/authorities/names/label/'
    #URL-encode question mark character to support searching APs with '?' in string
    lcnafAP_URL_encoded = lcnafAP.replace("?", "%3F" )
   
    full_query = (BASE_PR_URL + lcnafAP_URL_encoded)
    print(full_query)
    ...
    if r.status_code == 200:
        ...
        founddiv = soup.find("div", { "id" : "tab1" })

        LOCurl = founddiv['about']
        #get ID from URL
        lcnafID = LOCurl.split('/')[-1]
        print(lcnafID)
        found = "Y"
        #call next function
        Check_ID_LOC_info(lcnafID, lcnafAP, found)
    ...
    if r.status_code == 404:
        ...
        lcnafID = ""
        #add to spreadsheet
        sheet.append([lcnafAP, found])
        wb.save(spreadsheet_filename)

#checks LCNAF authority record
def Check_ID_LOC_info(lcnafID, lcnafAP, found):
    recordmarcURL = "https://id.loc.gov/authorities/names/" + lcnafID + ".marcxml.xml"
    ...
    Check_VIAF(lcnafID, lcnafAP, found, id_dict, RDArecord)


def Check_VIAF(lcnafID, lcnafAP, found, id_dict, RDArecord):

    #get the links to clustered identifiers
    urllc = "https://www.viaf.org/viaf/lccn/" + lcnafID + '/justlinks.json' 
    ...
    try:
        ...
        if not id_dict["wikidata"] == "":
            ...
            if not id_dict["wikidata"] == j["WKP"][0]:
                ...
                output_different_ids.write(lcnafID + " and " + j["viafID"] + " have different wikidata identifiers." + "\n")
    ...
    if isni == wikidata == viaf == "":
        ...
        output_existing.write(lcnafID + " already has the available identifiers found on viaf." + "\n")
    ...
    #outputs file for Connexion macro - it is outputting only those available on viaf, and not already in id.loc.gov records
        ...
        output_viaf.write(lcnafID + viaf + isni + wikidata + "\n")

    time.sleep(1) 
    
    #add to spreadsheet
    sheet.append([lcnafAP, found, lcnafID, RDArecord, str(id_dict), viaf + isni + wikidata, wikilabel])
    wb.save(spreadsheet_filename)

    #print(lcnafID, viaf, isni, wikidata)
...
#create spreadsheet that goves over of results
spreadsheet_filename = '1000_access_point_and_identifier_check_results.xlsx'
